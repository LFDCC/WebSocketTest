...
namespace WebSocketTest.api
{
    public class TestController : ApiController
    {
        ...
        public HttpResponseMessage Connect(string nickName)
        {
            ...
            {
                HttpContext.Current.AcceptWebSocketRequest(ProcessChat); //在服务器端接受Web Socket请求，传入的函数作为Web Socket的处理函数，待Web Socket建立后该函数会被调用，在该函数中可以对Web Socket进行消息收发

                return Request.CreateResponse(HttpStatusCode.SwitchingProtocols); //构造同意切换至Web Socket的Response.
            }
            else
            {
                return new HttpResponseMessage { Content = new StringContent("{\"status\":200,\"message\":\"非法链接！\"}", Encoding.UTF8, "application/json") };
            }
        }
